...
The fast method takes advantage of Monte Carlo methods \cite{barbu2020monte} to generate the time series. The idea behinf this procedure is to compute the inter-event time instead 
...
With that we can calculate the inter-event time following the next steps. 
...
To conclude the time series generation section, we can generalize the algorithm in order to generate $M$ Hawkes processes coupled \cite{dassios2013exact,laub2021elements}. 
...
Beginning with the first configuration, we have a homogeneous Poisson process, which we know that its interevent time will be distributed randomly with a probability of having an inter-event
time $x_i$ given by $P(x_i)=\mu e^{-\mu x_i}$. Consequently, two consecutive events will be part of a cluster, fixing the resolution parameter to $\Delta$ with a probability of
\begin{equation}
